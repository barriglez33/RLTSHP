...
def ensure_translation(a):
    ...
    if a.get('translation_status')=='translated' and a.get('rss_title') and a.get('rss_body'):
        return
...
def main():
    ...
    for item in CFG['keywords']:
        ...
        if item['category']=='Horoscopes & Astrology': print('  Astrology query:',search_query(item))
        ...
        for c in unique.values():
            ...
            full=ex['title']+'\n'+ex['body']
            if not keyword_matches(full,c['keyword'],item) or not has_context(full,item): continue
            pub=c['published']

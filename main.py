import json, re, hashlib, html, unicodedata
...
def search_query(item):
    return f'"{item["keyword"]}" (relationship OR relationships OR dating OR romance OR love OR couples OR marriage OR breakup OR divorce OR intimacy)'
    ...
    for ed in CFG['google_news_editions']:
        ...
        for e in list(getattr(feed,'entries',[]))[:CFG['settings']['google_results_per_edition']]:
            ...
            try:
                ...
                pass
            out.append(dict(url=u,title=getattr(e,'title',''),source=src or domain(u),published=feed_dt(e),
                            language='',country=ed['label'],via='Google News',keyword=item['keyword'],category=item['category']))
    return out
...
def translate(text):
    try:
        tr=GoogleTranslator(source='auto',target=CFG['translation']['target_language'])
        return '\n\n'.join(tr.translate(c) or c for c in split_chunks(text,CFG['translation']['chunk_size'])), True
    except Exception:
        return text, False
...
def main():
    ...
    for item in CFG['keywords']:
        print('SEARCH:',item['keyword'])
        candidates=discover_gdelt(item)+discover_google(item)
        unique={c['url']:c for c in candidates if c['published']>=cutoff}
        for c in unique.values():
            if c['url'] in byurl:
                a=byurl[c['url']]
                if c['keyword'] not in a.setdefault('matched_keywords',[]): a['matched_keywords'].append(c['keyword'])
                if c['category'] not in a.setdefault('categories',[]): a['categories'].append(c['category'])
                continue
            ex=extract(c['url'])
            if not ex or len(ex['body'])<CFG['settings']['minimum_body_characters']: continue
            full=ex['title']+'\n'+ex['body']
            if not keyword_matches(full,c['keyword']) or not has_context(full): continue
            pub=c['published']
            a={'id':hashlib.sha256(c['url'].encode()).hexdigest()[:20],'title':ex['title'] or c['title'],'source':c['source'] or domain(c['url']),'author':ex['author'],'url':c['url'],'published_iso':pub.isoformat(),'published_rfc2822':format_datetime(pub),'body':ex['body'],'matched_keywords':[c['keyword']],'categories':[c['category']],'source_languages':[c['language']] if c['language'] else [],'source_countries':[c['country']] if c['country'] else [],'discovery_sources':[c['via']]}
